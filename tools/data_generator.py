...
DETERMINISTIC_BASE_TIMESTAMP_MS = 1704067200000  # 2024-01-01T00:00:00Z
...
def gaussian_random(
    ...
    return random_source.gauss(mean, stddev)
...
def random_phone(rng: Optional[random.Random] = None) -> str:
    random_source = rng if rng is not None else random
    return (
        f"+1-{random_source.randint(200, 999)}-"
        f"{random_source.randint(100, 999)}-"
        f"{random_source.randint(1000, 9999)}"
    )
...
def random_email(
    ...
    random_source = rng if rng is not None else random
    domain = random_source.choice(DOMAINS)
    pattern = random_source.choice([
        f"{first.lower()}.{last.lower()}",
        f"{first.lower()}{last.lower()}",
        f"{first[0].lower()}{last.lower()}",
        f"{last.lower()}.{first.lower()}",
        f"{first.lower()}{random_source.randint(1, 999)}",
    ])
...
def random_datetime(
    ...
    return start + timedelta(
        seconds=random_source.randint(0, int(delta.total_seconds()))
    )
...
class DataGenerator:
    ...
    def generate_ticks(self, instrument_symbol: str, count: int = 1000) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            ...
            tick = {
                ...
                "timestamp": (
                    DETERMINISTIC_BASE_TIMESTAMP_MS - (count - i) * 1000
                ),
            }
            ticks.append(tick)
    ...
    def generate_candles(self, instrument_symbol: str, interval_minutes: int = 60,
        ...
        candles = []
        price = instrument["price"]
        now = DETERMINISTIC_BASE_TIMESTAMP_MS
